...
folder are mapped to the application's root namespace, 'StoreException'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "StoreException"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'StoreException' in the same way
as `"StoreException/sass/src"`.
